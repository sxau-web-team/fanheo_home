...
class WeixinController extends Controller {
    	...
    	if(mc_user_id()) {
    		if(mc_is_admin()) {
	    		if($_POST['weixin_appid'] && $_POST['weixin_appsecret'] && $_POST['weixin_token']) {
			    	...
			    	$this->theme(mc_option('theme'))->display('Control/weixin');
		    	}
	    	} else {
    ...
    public function qunfa(){
    	if(mc_user_id()) {
		   ...
		   "msgtype": "text", "text": { "content": "'.$_POST['content'].'"}
					...
					$this->theme(mc_option('theme'))->display('Control/qunfa');
				};
	    	} else {
		    	$this->error('您没有权限访问此页面！');
	    	};
    	...
    	if(mc_user_id()) {
    		if(mc_is_admin()) {
	    		if($_POST['msg'] && $_POST['return']) {
	    			...
	    			M('option')->where("id = '".$_GET['del']."'")->delete();
		    		$this->success('删除成功！',U('control/weixin/huifu'));
	    		} else {
		    		$this->theme(mc_option('theme'))->display('Control/huifu');
	    		}
	    	} else {
		    	$this->error('您没有权限访问此页面！');
	    	};
    ...
    public function add_weixin(){
        if(mc_is_admin() && $_POST['fileup']=='ok') {
	        ...
	        $filex = $new_files;
	        $access_token = mc_get_access_token('wxe3e39b8611d7800f','56683e56a963842adf1e02cb3fd92957');
	        $callback = mc_upload_media($filex,$access_token);
		    $this->assign('callback',$callback);
	        $this->theme(mc_option('theme'))->display('Publish/add_weixin');
	     } else {
...
<FromUserName>gh_fc366c37921d</FromUserName>
