...
class BackupsqlController extends Controller{
	...
	function tablist() {
		...
		$this->theme(mc_option('theme'))->display('Admin/Backupsql/tablist');
	}
	public function backupsql() {
		$this->assign('title','饭盒后台管理系统-数据库管理-备份数据库');
		$this->assign('loginname',$_SESSION['AdminUser']);
		$this->tables =$this->db->getTables($this->dbName);
		$this->theme(mc_option('theme'))->display('Admin/Backupsql/backup');
	}
	/**
     * 导出SQL文件
     * @access public
